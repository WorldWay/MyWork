...
public interface KeepAliveRequestTimeoutHandler {
    ...
    KeepAliveRequestTimeoutHandler EXCEPTION = new KeepAliveRequestTimeoutHandler() {
        public void keepAliveRequestTimedOut(KeepAliveFilter filter, IoSession session) throws Exception {
    		...
    		sessionMap.delSession(session.getRemoteAddress().toString()
    				.substring(1));
    ...
    KeepAliveRequestTimeoutHandler CLOSE = new KeepAliveRequestTimeoutHandler() {
        ...
        public void keepAliveRequestTimedOut(KeepAliveFilter filter, IoSession session) throws Exception {
    		...
    		sessionMap.delSession(session.getRemoteAddress().toString()
    				.substring(1));
        	LOGGER.warn(session.getRemoteAddress().toString()
    				.substring(1)+"由于在 {} second(s)未收到心跳包，因此关闭了连接.", filter.getRequestTimeout());            
            session.closeNow();
        }
    };
